function [m, s, m, s, grand, grand, bc, allTrSpikes, allTrChoice] = twagCP(cpPlot, channelNumber, spikeTimes, pp, centerTime, goodtrial, dirPref)
...
cp.channel.allRevco.m      = cell(nCh, 1);
cp.channel.allRevco.s      = cell(nCh, 1);
cp.channel.allRevco.grand  = nan(nCh, 1);

cp.channel.frozen.m      = cell(nCh, 1);
cp.channel.frozne.s      = cell(nCh, 1);
cp.channel.frozen.grand  = nan(nCh, 1);
...
for jj = channelNumber(kk):channelNumber(nCh)
    ...
    [m{jj,1}, s{jj,1}] = choiceProbabilityCalculate(allTrSpikes{jj,1}, allTrChoice{jj,1});
    ...
    grand(jj) = mean(m{jj,1}(350:1400));
    
    if cpPlot == 1
        text(0.1, max(m{jj,1}), num2str(round(grand(jj),2)));
    end
