function [syncClock] = syncPlexonClock(PDS, plxname)
%load events only
plx=ephys.readPlx(plxname, false); % 2nd arg must be true if you want spikes
switchbits=false;
invertedBits=false;
%%
...
syncClock.PL2PTBfit=[flagPlexonTrialStartTime(validField) flagPlexonTrialStartTime(validField)*0+1]\flagComputerTrialStartTime(validField);
PL2PTB=@(x) x*syncClock.PL2PTBfit(1) + syncClock.PL2PTBfit(2);
PTB2PL=@(x) (x - syncClock.PL2PTBfit(2))/syncClock.PL2PTBfit(1);
...
% rename stuff
syncClock.PL2PTB = PL2PTB;
syncClock.PTB2PL = PTB2PL;
% syncClock.PL2PTBfit = PL2PTBfit;
syncClock.error = maxreconstructionerror;
syncClock.plexonTrialStartTime = flagPlexonTrialStartTime;
syncClock.computerTrialStartTime = flagComputerTrialStartTime;
syncClock.datapixxTrialStartTime = flagDatapixxTrialStartTime;
